...
class MovingAverageStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        data_card_index = self.create_data_index(
            data_card.product.exch,
            data_card.product.name,
            data_card.freq,
            aggregation=data_card.aggregation,
        )
        self.data = self.datas[data_card_index]
        self.sma_indicator = SmaIndicator(
            data=self.data, min_period=self.params['period']
        )
    ...
    def next(self):
        ...
        #     print('close[-1]={} sma[-1]={} close[-2]={} sma[-2]={}'.format(
        #         self.data[-1].close, self.sma_indicator.sma[-1],
        #         self.data[-2].close, self.sma_indicator.sma[-2])
        #     )
        ...
        if (
            self.data[-1].close > self.sma_indicator.sma[-1]
            and self.data[-2].close <= self.sma_indicator.sma[-2]
        ):
            self.buy(
